...
function makeMaterial(highlighted: boolean, risk: HighlightRisk = "none"): THREE.ShaderMaterial {
  ...
  // part itself communicates threat, not just the HUD banner. The
  // rest of the material (transparent + depthWrite=false) is
  // untouched — the rotation flicker is accepted as hologram
  // character per the Apr 19 testing session.
  const risky = highlighted && RISKY.has(risk);
  const baseColor = risky
    ? new THREE.Color(1.0, 0.47, 0.31)  // rgb(255,120,80) — HAL visualizer ring
...
function HologramModel({
  ...
  // Re-memo highlighted material when risk changes so Fresnel palette
  // swaps blue↔red as the alert severity changes.
