...
6. Hybrid routing is **cloud-first** when `CLOUD_FIRST=true` in `server/.env`. `server/cactus_proxy.py` POSTs to Cactus's proxy at `https://104.198.76.3/api/v1` (endpoint `/text` for text turns, `/omni` for audio). On success, local is never touched. On timeout/network/http error, `server.py` falls back to `state.llm.complete()` for the same turn. Matches the behind-the-Moon story: cloud when connected, on-device when not. Cloud model is `CACTUS_CLOUD_MODEL` (default `gemini-3.1-pro-preview` for audio-comprehension accuracy on voice turns — flash-lite has been observed to mis-identify less-common proper nouns like Kibo module names; drop to `gemini-3.1-flash-lite-preview` if you want ~1 s roundtrip instead of ~2 s and can live with the accuracy trade-off). Auth via `CACTUS_CLOUD_KEY` loaded from `server/.env` by `python-dotenv` at `config.py` import. When `CLOUD_FIRST=false`, the code path is pure local — no cloud attempts at all. Response always carries `source: "local"|"cloud"`.
